...
```{figure} ./img/bigOgraph.png
    In this figure, $n_{0}$ -> Threshold size of input (i.e. minimum input size, after which functions (algorithms) show their true behavior)    f(n) -> Our function (algorithm) which is under analysis.     g(n) -> Another function (algorithm) having higher rate of growth than our function (algorithm)
```
...
2. 0 (zero) means, there is no edge between those nodes
