...
##### Operations on stack
1. **Push**
    1. It refers to storing element on the top of the stack.
    2. During push, TOP get incremented by 1, and we store element at that location, with:
        ```{code} 
        TOP++;
        stack[TOP] = element;
        ```
2. **Pop**
    1. It refers to removing element from the top of the stack.
    2. During pop, we simply decrement TOP by 1
        ```{code}
        TOP--;
        ```
3. **Peek**
    1. It refers to view the TOP element of stack, with:
    ```{code}
        stack[TOP];
    ```
